...
namespace LocationTester
{
    class Server
    {
        ...
        {
            IPAddress serverAdd = IPAddress.Parse("10.0.0.197");
            int port = 7592;
            TcpListener listner = new TcpListener(serverAdd, 7598);
            listner.Start();
            ...
            while (soc.Connected)
            {
                ...
                string afterPrase;
                int k = soc.Receive(b);
                Console.WriteLine(DateTime.Now.ToString("hh:mm:ss")+"Recieved..." + k + " bytes");
